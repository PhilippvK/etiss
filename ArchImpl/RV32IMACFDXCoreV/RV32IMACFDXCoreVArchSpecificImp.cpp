 ...
 *
 * This file contains the architecture specific implementation for the RV32IMACFDXCoreV
 * core architecture.
 *
...
#include "RV32IMACFDXCoreVArchSpecificImp.h"
...
*/
etiss::int32 RV32IMACFDXCoreVArch::handleException(etiss::int32 cause, ETISS_CPU * cpu)
{
...
*/
void RV32IMACFDXCoreVArch::initInstrSet(etiss::instr::ModedInstructionSet & mis) const
{
    ...
    if (false) {
        ...
        etiss::instr::ModedInstructionSet iset("RV32IMACFDXCoreVISA");
		bool ok = true;
		RV32IMACFDXCoreVISA.addTo(iset,ok);
	...
	bool ok = true;
	RV32IMACFDXCoreVISA.addTo(mis,ok);
	if (!ok)
		etiss::log(etiss::FATALERROR,"Failed to add instructions for RV32IMACFDXCoreVISA");
...
// -----------------------------------------------------------------------------
    ...
    vis->length_updater_ = [](VariableInstructionSet &, InstructionContext &ic, BitArray &ba) {
        std::function<void(InstructionContext & ic, etiss_uint32 opRd)> updateRV32IMACFDXCoreVInstrLength =
        ...
        case 2:
            ...
            {
                updateRV32IMACFDXCoreVInstrLength(ic, opRd);
                break;
            }
        case 4:
...
*/
void RV32IMACFDXCoreVArch::compensateEndianess(ETISS_CPU * cpu, etiss::instr::BitArray & ba) const
{
	/**************************************************************************
...
std::shared_ptr<etiss::VirtualStruct> RV32IMACFDXCoreVArch::getVirtualStruct(ETISS_CPU * cpu)
{
	...
	for (uint32_t i = 0; i < 32; ++i){
		ret->addField(new RegField_RV32IMACFDXCoreV(*ret,i));
	}

	ret->addField(new pcField_RV32IMACFDXCoreV(*ret));
	return ret;
}
...
/**
	...
	@details Interrupt vector table is used to inform the core whenever an edge/level triggered interrupt
				...
				lines.
*/
etiss::InterruptVector * RV32IMACFDXCoreVArch::createInterruptVector(ETISS_CPU * cpu)
{
	if (cpu == 0)
		return 0;
	...
	vec.push_back(&((RV32IMACFDXCoreV*)cpu)->MIE);
	mask.push_back(&((RV32IMACFDXCoreV*)cpu)->MIP);

	return new etiss::MappedInterruptVector<etiss::uint32>(vec, mask);
}

void RV32IMACFDXCoreVArch::deleteInterruptVector(etiss::InterruptVector * vec, ETISS_CPU * cpu)
{
	delete vec;
}

etiss::InterruptEnable* RV32IMACFDXCoreVArch::createInterruptEnable(ETISS_CPU* cpu) {
 	return new etiss::MappedInterruptEnable<etiss::uint32>(&((RV32IMACFDXCoreV*)cpu)->MSTATUS, 15);
}

void RV32IMACFDXCoreVArch::deleteInterruptEnable(etiss::InterruptEnable* en, ETISS_CPU* cpu) {
	delete en;
}
