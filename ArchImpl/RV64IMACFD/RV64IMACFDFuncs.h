...
static inline void raise(ETISS_CPU * const cpu, ETISS_System * const system, void * const * const plugin_pointers, etiss_int32 irq, etiss_uint64 mcause);
...
static inline etiss_uint64 calc_irq_mcause(ETISS_CPU * const cpu, ETISS_System * const system, void * const * const plugin_pointers);
...
if ((get_field(*((RV64IMACFD*)cpu)->CSR[384LL], 17293822569102704640ULL))) { // conditional
mask = mask | 262144LL;
} // conditional
} // block
} // conditional
...
*((RV64IMACFD*)cpu)->CSR[768LL] = val & mstatus_mask(cpu, system, plugin_pointers);
} // conditional
 else if (csr == 256LL) { // conditional
*((RV64IMACFD*)cpu)->CSR[768LL] = val & sstatus_mask(cpu, system, plugin_pointers);
} // conditional
...
etiss_uint64 s = csr_read(cpu, system, plugin_pointers, 256LL);
s = set_field(s, 32LL, get_field(s, 2LL));
s = set_field(s, 256LL, ((RV64IMACFD*)cpu)->PRIV);
...
etiss_uint64 s = csr_read(cpu, system, plugin_pointers, 768LL);
s = set_field(s, 128LL, get_field(s, 8LL));
s = set_field(s, 6144LL, ((RV64IMACFD*)cpu)->PRIV);
...
etiss_uint64 mie = get_field(*((RV64IMACFD*)cpu)->CSR[768LL], 8LL);
...
etiss_uint64 sie = get_field(csr_read(cpu, system, plugin_pointers, 256LL), 2LL);
...
return 9223372036854775808ULL | ctz(enabled_interrupts);
} // block
} // conditional
return 0ULL;
} // block
...
{
{ // block
etiss_uint64 irq_mcause = calc_irq_mcause(cpu, system, plugin_pointers);
