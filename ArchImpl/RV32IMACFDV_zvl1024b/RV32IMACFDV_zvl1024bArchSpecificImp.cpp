 ...
 *
 * This file contains the architecture specific implementation for the RV32IMACFDV_zvl1024b
 * core architecture.
 *
...
#include "RV32IMACFDV_zvl1024bArchSpecificImp.h"
...
*/
etiss::int32 RV32IMACFDV_zvl1024bArch::handleException(etiss::int32 cause, ETISS_CPU * cpu)
{
	RV32IMACFDV_zvl1024b_translate_exc_code(cpu, nullptr, nullptr, cause);
...
*/
void RV32IMACFDV_zvl1024bArch::initInstrSet(etiss::instr::ModedInstructionSet & mis) const
{
    ...
    if (false) {
        ...
        etiss::instr::ModedInstructionSet iset("RV32IMACFDV_zvl1024bISA");
		bool ok = true;
		RV32IMACFDV_zvl1024bISA.addTo(iset,ok);
	...
	bool ok = true;
	RV32IMACFDV_zvl1024bISA.addTo(mis,ok);
	if (!ok)
		etiss::log(etiss::FATALERROR,"Failed to add instructions for RV32IMACFDV_zvl1024bISA");
...
{ // procedure
cp.code() += "{ // procedure\n";
cp.code() += "RV32IMACFDV_zvl1024b_translate_exc_code(cpu, system, plugin_pointers, " + std::to_string(error_code) + "ULL);\n";
...
// -----------------------------------------------------------------------------
    ...
    vis->length_updater_ = [](VariableInstructionSet &, InstructionContext &ic, BitArray &ba) {
        std::function<void(InstructionContext & ic, etiss_uint32 opRd)> updateRV32IMACFDV_zvl1024bInstrLength =
        ...
        case 2:
            ...
            {
                updateRV32IMACFDV_zvl1024bInstrLength(ic, opRd);
                break;
            }
        case 4:
...
*/
void RV32IMACFDV_zvl1024bArch::compensateEndianess(ETISS_CPU * cpu, etiss::instr::BitArray & ba) const
{
	/**************************************************************************
...
std::shared_ptr<etiss::VirtualStruct> RV32IMACFDV_zvl1024bArch::getVirtualStruct(ETISS_CPU * cpu)
{
	...
	for (uint32_t i = 0; i < 32; ++i){
		ret->addField(new RegField_RV32IMACFDV_zvl1024b(*ret,i));
	}

	ret->addField(new pcField_RV32IMACFDV_zvl1024b(*ret));

	for (uint32_t i = 0; i < 32; ++i){
		ret->addField(new FloatRegField_RV32IMACFDV_zvl1024b(*ret,i));
	}

	//for (uint32_t i = 0; i < 4; ++i){
	//	ret->addField(new CSRField_RV32IMACFDV_zvl1024b(*ret,i));
	//}

	// FCSR
	ret->addField(new CSRField_RV32IMACFDV_zvl1024b(*ret,1));
	ret->addField(new CSRField_RV32IMACFDV_zvl1024b(*ret,2));
	ret->addField(new CSRField_RV32IMACFDV_zvl1024b(*ret,3));
	// VCSR
	ret->addField(new CSRField_RV32IMACFDV_zvl1024b(*ret,8));
	ret->addField(new CSRField_RV32IMACFDV_zvl1024b(*ret,9));
	ret->addField(new CSRField_RV32IMACFDV_zvl1024b(*ret,10));
	ret->addField(new CSRField_RV32IMACFDV_zvl1024b(*ret,15));
	// M CSR
	ret->addField(new CSRField_RV32IMACFDV_zvl1024b(*ret,768));
	ret->addField(new CSRField_RV32IMACFDV_zvl1024b(*ret,769));
	ret->addField(new CSRField_RV32IMACFDV_zvl1024b(*ret,770));
	ret->addField(new CSRField_RV32IMACFDV_zvl1024b(*ret,771));
	ret->addField(new CSRField_RV32IMACFDV_zvl1024b(*ret,772));
	ret->addField(new CSRField_RV32IMACFDV_zvl1024b(*ret,773));
	ret->addField(new CSRField_RV32IMACFDV_zvl1024b(*ret,774));
	ret->addField(new CSRField_RV32IMACFDV_zvl1024b(*ret,832));
	ret->addField(new CSRField_RV32IMACFDV_zvl1024b(*ret,833));
	ret->addField(new CSRField_RV32IMACFDV_zvl1024b(*ret,834));
	ret->addField(new CSRField_RV32IMACFDV_zvl1024b(*ret,835));
	ret->addField(new CSRField_RV32IMACFDV_zvl1024b(*ret,836));
	//
	ret->addField(new CSRField_RV32IMACFDV_zvl1024b(*ret,3072));
	ret->addField(new CSRField_RV32IMACFDV_zvl1024b(*ret,3073));
	ret->addField(new CSRField_RV32IMACFDV_zvl1024b(*ret,3074));
	// VCSR
	ret->addField(new CSRField_RV32IMACFDV_zvl1024b(*ret,3104));
	ret->addField(new CSRField_RV32IMACFDV_zvl1024b(*ret,3105));
	ret->addField(new CSRField_RV32IMACFDV_zvl1024b(*ret,3106));
	//
	ret->addField(new CSRField_RV32IMACFDV_zvl1024b(*ret,3200));
	ret->addField(new CSRField_RV32IMACFDV_zvl1024b(*ret,3201));
	ret->addField(new CSRField_RV32IMACFDV_zvl1024b(*ret,3202));
	// M CSR
	ret->addField(new CSRField_RV32IMACFDV_zvl1024b(*ret,3857));
	ret->addField(new CSRField_RV32IMACFDV_zvl1024b(*ret,3858));
	ret->addField(new CSRField_RV32IMACFDV_zvl1024b(*ret,3859));
	ret->addField(new CSRField_RV32IMACFDV_zvl1024b(*ret,3860));
	// V0-V31
	for (uint32_t i = 0; i < 32; ++i){
		ret->addField(new VectorRegField_RV32IMACFDV_zvl1024b(*ret,i));
	}
...
/**
	...
	@details Interrupt vector table is used to inform the core whenever an edge/level triggered interrupt
				...
				lines.
*/
etiss::InterruptVector * RV32IMACFDV_zvl1024bArch::createInterruptVector(ETISS_CPU * cpu)
{
	if (cpu == 0)
		return 0;
	...
	vec.push_back(&((RV32IMACFDV_zvl1024b*)cpu)->MIP);
	mask.push_back(&((RV32IMACFDV_zvl1024b*)cpu)->MIE);

	return new etiss::MappedInterruptVector<etiss::uint32>(vec, mask);
}

void RV32IMACFDV_zvl1024bArch::deleteInterruptVector(etiss::InterruptVector * vec, ETISS_CPU * cpu)
{
	delete vec;
}

etiss::InterruptEnable* RV32IMACFDV_zvl1024bArch::createInterruptEnable(ETISS_CPU* cpu) {
 	return new etiss::MappedInterruptEnable<etiss::uint32>(&((RV32IMACFDV_zvl1024b*)cpu)->MSTATUS, 15);
}

void RV32IMACFDV_zvl1024bArch::deleteInterruptEnable(etiss::InterruptEnable* en, ETISS_CPU* cpu) {
	delete en;
}
