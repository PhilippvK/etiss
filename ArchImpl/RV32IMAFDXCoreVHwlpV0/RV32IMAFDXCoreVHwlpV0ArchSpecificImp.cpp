 ...
 *
 * This file contains the architecture specific implementation for the RV32IMAFDXCoreVHwlpV0
 * core architecture.
 *
...
#include "RV32IMAFDXCoreVHwlpV0ArchSpecificImp.h"
...
*/
etiss::int32 RV32IMAFDXCoreVHwlpV0Arch::handleException(etiss::int32 cause, ETISS_CPU * cpu)
{
...
*/
void RV32IMAFDXCoreVHwlpV0Arch::initInstrSet(etiss::instr::ModedInstructionSet & mis) const
{
    ...
    if (false) {
        ...
        etiss::instr::ModedInstructionSet iset("RV32IMAFDXCoreVHwlpV0ISA");
		bool ok = true;
		RV32IMAFDXCoreVHwlpV0ISA.addTo(iset,ok);
	...
	bool ok = true;
	RV32IMAFDXCoreVHwlpV0ISA.addTo(mis,ok);
	if (!ok)
		etiss::log(etiss::FATALERROR,"Failed to add instructions for RV32IMAFDXCoreVHwlpV0ISA");
...
// -----------------------------------------------------------------------------
    ...
    vis->length_updater_ = [](VariableInstructionSet &, InstructionContext &ic, BitArray &ba) {
        std::function<void(InstructionContext & ic, etiss_uint32 opRd)> updateRV32IMAFDXCoreVHwlpV0InstrLength =
        ...
        case 2:
            ...
            {
                updateRV32IMAFDXCoreVHwlpV0InstrLength(ic, opRd);
                break;
            }
        case 4:
...
*/
void RV32IMAFDXCoreVHwlpV0Arch::compensateEndianess(ETISS_CPU * cpu, etiss::instr::BitArray & ba) const
{
	/**************************************************************************
...
std::shared_ptr<etiss::VirtualStruct> RV32IMAFDXCoreVHwlpV0Arch::getVirtualStruct(ETISS_CPU * cpu)
{
	...
	for (uint32_t i = 0; i < 32; ++i){
		ret->addField(new RegField_RV32IMAFDXCoreVHwlpV0(*ret,i));
	}

	ret->addField(new pcField_RV32IMAFDXCoreVHwlpV0(*ret));
	return ret;
}
...
/**
	...
	@details Interrupt vector table is used to inform the core whenever an edge/level triggered interrupt
				...
				lines.
*/
etiss::InterruptVector * RV32IMAFDXCoreVHwlpV0Arch::createInterruptVector(ETISS_CPU * cpu)
{
	if (cpu == 0)
		return 0;
	...
	vec.push_back(&((RV32IMAFDXCoreVHwlpV0*)cpu)->MIE);
	mask.push_back(&((RV32IMAFDXCoreVHwlpV0*)cpu)->MIP);

	return new etiss::MappedInterruptVector<etiss::uint32>(vec, mask);
}

void RV32IMAFDXCoreVHwlpV0Arch::deleteInterruptVector(etiss::InterruptVector * vec, ETISS_CPU * cpu)
{
	delete vec;
}

etiss::InterruptEnable* RV32IMAFDXCoreVHwlpV0Arch::createInterruptEnable(ETISS_CPU* cpu) {
 	return new etiss::MappedInterruptEnable<etiss::uint32>(&((RV32IMAFDXCoreVHwlpV0*)cpu)->MSTATUS, 15);
}

void RV32IMAFDXCoreVHwlpV0Arch::deleteInterruptEnable(etiss::InterruptEnable* en, ETISS_CPU* cpu) {
	delete en;
}
