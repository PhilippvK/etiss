 ...
 *
 * This file contains the architecture specific implementation for the RV32IMAFDXCoreVHwlp
 * core architecture.
 *
...
#include "RV32IMAFDXCoreVHwlpArchSpecificImp.h"
...
*/
etiss::int32 RV32IMAFDXCoreVHwlpArch::handleException(etiss::int32 cause, ETISS_CPU * cpu)
{
...
*/
void RV32IMAFDXCoreVHwlpArch::initInstrSet(etiss::instr::ModedInstructionSet & mis) const
{
    ...
    if (false) {
        ...
        etiss::instr::ModedInstructionSet iset("RV32IMAFDXCoreVHwlpISA");
		bool ok = true;
		RV32IMAFDXCoreVHwlpISA.addTo(iset,ok);
	...
	bool ok = true;
	RV32IMAFDXCoreVHwlpISA.addTo(mis,ok);
	if (!ok)
		etiss::log(etiss::FATALERROR,"Failed to add instructions for RV32IMAFDXCoreVHwlpISA");
...
// -----------------------------------------------------------------------------
    ...
    vis->length_updater_ = [](VariableInstructionSet &, InstructionContext &ic, BitArray &ba) {
        std::function<void(InstructionContext & ic, etiss_uint32 opRd)> updateRV32IMAFDXCoreVHwlpInstrLength =
        ...
        case 2:
            ...
            {
                updateRV32IMAFDXCoreVHwlpInstrLength(ic, opRd);
                break;
            }
        case 4:
...
*/
void RV32IMAFDXCoreVHwlpArch::compensateEndianess(ETISS_CPU * cpu, etiss::instr::BitArray & ba) const
{
	/**************************************************************************
...
std::shared_ptr<etiss::VirtualStruct> RV32IMAFDXCoreVHwlpArch::getVirtualStruct(ETISS_CPU * cpu)
{
	...
	for (uint32_t i = 0; i < 32; ++i){
		ret->addField(new RegField_RV32IMAFDXCoreVHwlp(*ret,i));
	}

	ret->addField(new pcField_RV32IMAFDXCoreVHwlp(*ret));
	return ret;
}
...
/**
	...
	@details Interrupt vector table is used to inform the core whenever an edge/level triggered interrupt
				...
				lines.
*/
etiss::InterruptVector * RV32IMAFDXCoreVHwlpArch::createInterruptVector(ETISS_CPU * cpu)
{
	if (cpu == 0)
		return 0;
	...
	vec.push_back(&((RV32IMAFDXCoreVHwlp*)cpu)->MIE);
	mask.push_back(&((RV32IMAFDXCoreVHwlp*)cpu)->MIP);

	return new etiss::MappedInterruptVector<etiss::uint32>(vec, mask);
}

void RV32IMAFDXCoreVHwlpArch::deleteInterruptVector(etiss::InterruptVector * vec, ETISS_CPU * cpu)
{
	delete vec;
}

etiss::InterruptEnable* RV32IMAFDXCoreVHwlpArch::createInterruptEnable(ETISS_CPU* cpu) {
 	return new etiss::MappedInterruptEnable<etiss::uint32>(&((RV32IMAFDXCoreVHwlp*)cpu)->MSTATUS, 15);
}

void RV32IMAFDXCoreVHwlpArch::deleteInterruptEnable(etiss::InterruptEnable* en, ETISS_CPU* cpu) {
	delete en;
}
