 ...
 *
 * This file contains the architecture specific implementation for the RV32IMACFDK
 * core architecture.
 *
...
*/
etiss::int32 RV32IMACFDKArch::handleException(etiss::int32 cause, ETISS_CPU * cpu)
{
	RV32IMACFDK_translate_exc_code(cpu, nullptr, nullptr, cause);
...
void RV32IMACFDKArch::initInstrSet(etiss::instr::ModedInstructionSet & mis) const
{
    ...
    if (false) {
        ...
        etiss::instr::ModedInstructionSet iset("RV32IMACFDKISA");
		bool ok = true;
		RV32IMACFDKISA.addTo(iset,ok);
	...
	bool ok = true;
	RV32IMACFDKISA.addTo(mis,ok);
	if (!ok)
		etiss::log(etiss::FATALERROR,"Failed to add instructions for RV32IMACFDKISA");
...
{ // procedure
cp.code() += "{ // procedure\n";
cp.code() += "RV32IMACFDK_translate_exc_code(cpu, system, plugin_pointers, " + std::to_string(error_code) + "ULL);\n";
...
// -----------------------------------------------------------------------------
    ...
    vis->length_updater_ = [](VariableInstructionSet &, InstructionContext &ic, BitArray &ba) {
        std::function<void(InstructionContext & ic, etiss_uint32 opRd)> updateRV32IMACFDKInstrLength =
        ...
        case 2:
            ...
            {
                updateRV32IMACFDKInstrLength(ic, opRd);
                break;
            }
        case 4:
...
*/
void RV32IMACFDKArch::compensateEndianess(ETISS_CPU * cpu, etiss::instr::BitArray & ba) const
{
	/**************************************************************************
...
std::shared_ptr<etiss::VirtualStruct> RV32IMACFDKArch::getVirtualStruct(ETISS_CPU * cpu)
{
	...
	for (uint32_t i = 0; i < 32; ++i){
		ret->addField(new RegField_RV32IMACFDK(*ret,i));
	}

	ret->addField(new pcField_RV32IMACFDK(*ret));
	return ret;
}
...
/**
	...
	@details Interrupt vector table is used to inform the core whenever an edge/level triggered interrupt
				...
				lines.
*/
etiss::InterruptVector * RV32IMACFDKArch::createInterruptVector(ETISS_CPU * cpu)
{
	if (cpu == 0)
		return 0;
	...
	vec.push_back(&((RV32IMACFDK*)cpu)->MIP);
	mask.push_back(&((RV32IMACFDK*)cpu)->MIE);

	return new etiss::MappedInterruptVector<etiss::uint32>(vec, mask);
}

void RV32IMACFDKArch::deleteInterruptVector(etiss::InterruptVector * vec, ETISS_CPU * cpu)
{
	delete vec;
}

etiss::InterruptEnable* RV32IMACFDKArch::createInterruptEnable(ETISS_CPU* cpu) {
 	return new etiss::MappedInterruptEnable<etiss::uint32>(&((RV32IMACFDK*)cpu)->MSTATUS, 15);
}

void RV32IMACFDKArch::deleteInterruptEnable(etiss::InterruptEnable* en, ETISS_CPU* cpu) {
	delete en;
}
