...
{ // block
cp.code() += "{ // block\n";
cp.code() += "etiss_uint32 operation = *((RV32IMACFDK*)cpu)->X[10ULL];\n";
cp.code() += "etiss_uint32 parameter = *((RV32IMACFDK*)cpu)->X[11ULL];\n";
cp.code() += "*((RV32IMACFDK*)cpu)->X[10ULL] = (etiss_int32)(etiss_semihost(cpu, system, plugin_pointers, 32ULL, operation, parameter));\n";
cp.code() += "} // block\n";
} // block
cp.code() += "} // conditional\n";
