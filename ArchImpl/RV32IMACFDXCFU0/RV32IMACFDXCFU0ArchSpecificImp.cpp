 ...
 *
 * This file contains the architecture specific implementation for the RV32IMACFDXCFU0
 * core architecture.
 *
...
#include "RV32IMACFDXCFU0ArchSpecificImp.h"
...
*/
etiss::int32 RV32IMACFDXCFU0Arch::handleException(etiss::int32 cause, ETISS_CPU * cpu)
{
	RV32IMACFDXCFU0_translate_exc_code(cpu, nullptr, nullptr, cause);
...
*/
void RV32IMACFDXCFU0Arch::initInstrSet(etiss::instr::ModedInstructionSet & mis) const
{
    ...
    if (false) {
        ...
        etiss::instr::ModedInstructionSet iset("RV32IMACFDXCFU0ISA");
		bool ok = true;
		RV32IMACFDXCFU0ISA.addTo(iset,ok);
	...
	bool ok = true;
	RV32IMACFDXCFU0ISA.addTo(mis,ok);
	if (!ok)
		etiss::log(etiss::FATALERROR,"Failed to add instructions for RV32IMACFDXCFU0ISA");
...
{ // procedure
cp.code() += "{ // procedure\n";
cp.code() += "RV32IMACFDXCFU0_translate_exc_code(cpu, system, plugin_pointers, " + std::to_string(error_code) + "ULL);\n";
...
// -----------------------------------------------------------------------------
    ...
    vis->length_updater_ = [](VariableInstructionSet &, InstructionContext &ic, BitArray &ba) {
        std::function<void(InstructionContext & ic, etiss_uint32 opRd)> updateRV32IMACFDXCFU0InstrLength =
        ...
        case 2:
            ...
            {
                updateRV32IMACFDXCFU0InstrLength(ic, opRd);
                break;
            }
        case 4:
...
*/
void RV32IMACFDXCFU0Arch::compensateEndianess(ETISS_CPU * cpu, etiss::instr::BitArray & ba) const
{
	/**************************************************************************
...
std::shared_ptr<etiss::VirtualStruct> RV32IMACFDXCFU0Arch::getVirtualStruct(ETISS_CPU * cpu)
{
	...
	for (uint32_t i = 0; i < 32; ++i){
		ret->addField(new RegField_RV32IMACFDXCFU0(*ret,i));
	}

	ret->addField(new pcField_RV32IMACFDXCFU0(*ret));
	return ret;
}
...
/**
	...
	@details Interrupt vector table is used to inform the core whenever an edge/level triggered interrupt
				...
				lines.
*/
etiss::InterruptVector * RV32IMACFDXCFU0Arch::createInterruptVector(ETISS_CPU * cpu)
{
	if (cpu == 0)
		return 0;
	...
	vec.push_back(&((RV32IMACFDXCFU0*)cpu)->MIP);
	mask.push_back(&((RV32IMACFDXCFU0*)cpu)->MIE);

	return new etiss::MappedInterruptVector<etiss::uint32>(vec, mask);
}

void RV32IMACFDXCFU0Arch::deleteInterruptVector(etiss::InterruptVector * vec, ETISS_CPU * cpu)
{
	delete vec;
}

etiss::InterruptEnable* RV32IMACFDXCFU0Arch::createInterruptEnable(ETISS_CPU* cpu) {
 	return new etiss::MappedInterruptEnable<etiss::uint32>(&((RV32IMACFDXCFU0*)cpu)->MSTATUS, 15);
}

void RV32IMACFDXCFU0Arch::deleteInterruptEnable(etiss::InterruptEnable* en, ETISS_CPU* cpu) {
	delete en;
}
